 ...
 * @author julia
 */
public class InterpreteJpaController1 implements Serializable {

    public InterpreteJpaController1(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public void create(Interprete interprete) throws PreexistingEntityException, Exception {
        ...
        if (interprete.getCxiList() == null) {
            interprete.setCxiList(new ArrayList<Cxi>());
        }
        ...
        try {
            ...
            List<Cxi> attachedCxiList = new ArrayList<Cxi>();
            for (Cxi cxiListCxiToAttach : interprete.getCxiList()) {
                cxiListCxiToAttach = em.getReference(cxiListCxiToAttach.getClass(), cxiListCxiToAttach.getCxiPK());
                attachedCxiList.add(cxiListCxiToAttach);
            }
            interprete.setCxiList(attachedCxiList);
            ...
            for (Cxi cxiListCxi : interprete.getCxiList()) {
                Interprete oldInterpreteOfCxiListCxi = cxiListCxi.getInterprete();
                cxiListCxi.setInterprete(interprete);
                cxiListCxi = em.merge(cxiListCxi);
                if (oldInterpreteOfCxiListCxi != null) {
                    oldInterpreteOfCxiListCxi.getCxiList().remove(cxiListCxi);
                    oldInterpreteOfCxiListCxi = em.merge(oldInterpreteOfCxiListCxi);
                }
            }
    ...
    public void edit(Interprete interprete) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<Cxi> cxiListOld = persistentInterprete.getCxiList();
            List<Cxi> cxiListNew = interprete.getCxiList();
            ...
            for (Cxi cxiListOldCxi : cxiListOld) {
                if (!cxiListNew.contains(cxiListOldCxi)) {
                    if (illegalOrphanMessages == null) {
                        illegalOrphanMessages = new ArrayList<String>();
                    }
                    illegalOrphanMessages.add("You must retain Cxi " + cxiListOldCxi + " since its interprete field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            List<Cxi> attachedCxiListNew = new ArrayList<Cxi>();
            for (Cxi cxiListNewCxiToAttach : cxiListNew) {
                cxiListNewCxiToAttach = em.getReference(cxiListNewCxiToAttach.getClass(), cxiListNewCxiToAttach.getCxiPK());
                attachedCxiListNew.add(cxiListNewCxiToAttach);
            }
            cxiListNew = attachedCxiListNew;
            interprete.setCxiList(cxiListNew);
            ...
            for (Cxi cxiListNewCxi : cxiListNew) {
                if (!cxiListOld.contains(cxiListNewCxi)) {
                    Interprete oldInterpreteOfCxiListNewCxi = cxiListNewCxi.getInterprete();
                    cxiListNewCxi.setInterprete(interprete);
                    cxiListNewCxi = em.merge(cxiListNewCxi);
                    if (oldInterpreteOfCxiListNewCxi != null && !oldInterpreteOfCxiListNewCxi.equals(interprete)) {
                        oldInterpreteOfCxiListNewCxi.getCxiList().remove(cxiListNewCxi);
                        oldInterpreteOfCxiListNewCxi = em.merge(oldInterpreteOfCxiListNewCxi);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<Cxi> cxiListOrphanCheck = interprete.getCxiList();
            for (Cxi cxiListOrphanCheckCxi : cxiListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Interprete (" + interprete + ") cannot be destroyed since the Cxi " + cxiListOrphanCheckCxi + " in its cxiList field has a non-nullable interprete field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
